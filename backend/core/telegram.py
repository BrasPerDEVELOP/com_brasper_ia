...
async def _call(tenant: dict, method: str, payload: dict) -> dict:
    token = T.telegram_token(tenant)
    if not token:
        return {"ok": False, "reason": f"Tenant {tenant['id']}: Telegram sin bot_token configurado"}
...
def build_handoff_markup(tenant: dict) -> dict | None:
...
async def send_message(tenant: dict, chat_id, text: str, reply_markup: dict | None = None) -> dict:
    ...
    for idx, part in enumerate(parts):
        ...
        result = await _call(tenant, "sendMessage", payload)
    return result


async def send_photo(tenant: dict, chat_id, photo: str, caption: str = "") -> dict:
    ...
    return await _call(tenant, "sendPhoto", payload)
...
async def send_file_upload(tenant: dict, chat_id, filename: str, content: bytes,
...
async def send_typing(tenant: dict, chat_id) -> dict:
    return await _call(tenant, "sendChatAction", {"chat_id": chat_id, "action": "typing"})
...
def _allows_chat(tenant: dict, chat_type: str) -> bool:
...
async def process_update(tenant: dict, body: dict) -> dict:
    """Pipeline compartido por webhook y poller."""
    msg = parse_update(body)
    if not msg:
        return {"handled": False}
    if not _allows_chat(tenant, msg["chat_type"]):
    ...
    if msg.get("media"):
        ...
        if media["kind"] in ("voice", "audio") and audio_adapter.enabled(tenant):
            cid = db.get_or_create_conversation(tenant["id"], f"tg:{chat_id}", "telegram")
            if db.conversation_status(tenant["id"], cid) != "handoff":
                return await _handle_incoming_audio(tenant, chat_id, msg, cid)
        return await _handle_incoming_media(tenant, chat_id, msg)
    # Si un asesor atiende (handoff) no mostramos "escribiendo…": el bot está en pausa.
    cid = db.get_or_create_conversation(tenant["id"], f"tg:{chat_id}", "telegram")
    if db.conversation_status(tenant["id"], cid) != "handoff":
        await send_typing(tenant, chat_id)
    try:
        out = await engine.handle_message(tenant, f"tg:{chat_id}", msg["text"],
    ...
    await _send_banner(tenant, chat_id, cid, out.get("banner"))
    ...
    markup = build_handoff_markup(tenant) if out.get("handoff") else None
    await send_message(tenant, chat_id, out["response"], reply_markup=markup)
    return {"handled": True, "handoff": out.get("handoff", False), "new_lead": out.get("new_lead", False)}


async def _send_banner(tenant: dict, chat_id, cid: str, banner: dict | None) -> None:
    ...
    if image_url:
        r = await send_photo(tenant, chat_id, image_url, text)
        mref = sent_media_ref(r)
        media = {"provider": "telegram", "kind": "image", "ref": mref["ref"],
                 "caption": text} if mref and mref.get("ref") else None
        db.add_message(tenant["id"], cid, "assistant", text or "🎁 Banner primer envío", media=media)
    elif text:
        await send_message(tenant, chat_id, text)
        db.add_message(tenant["id"], cid, "assistant", text)
...
async def _handle_incoming_media(tenant: dict, chat_id, msg: dict) -> dict:
    ...
    if caption:
        text += f" — {caption}"
    db.add_message(tenant["id"], cid, "user", text, media={**media, "caption": caption})
    ...
    status = db.conversation_status(tenant["id"], cid)
    if status != "handoff":
        ...
        await send_message(tenant, chat_id, ack)
...
async def _handle_incoming_audio(tenant: dict, chat_id, msg: dict, cid: str) -> dict:
    ...
    media = msg["media"]
    content, ctype = await download_file(tenant, media["ref"])
    if not content:
        return await _handle_incoming_media(tenant, chat_id, msg)
    tr = await audio_adapter.transcribe_bytes(tenant, content, media.get("mime") or ctype or "audio/ogg")
    ...
    try:
        ...
        out = await engine.handle_message(tenant, f"tg:{chat_id}", text, channel="telegram",
...
async def download_file(tenant: dict, file_id: str) -> tuple[bytes | None, str | None]:
    """Descarga un archivo de Telegram por file_id (getFile + descarga con el token)."""
    token = T.telegram_token(tenant)
    if not token:
        return None, None
    info = await _call(tenant, "getFile", {"file_id": file_id})
    if not info.get("ok"):
        return None, None
...
# ---------- admin / diagnóstico ----------
async def get_me(tenant: dict) -> dict:
    return await _call(tenant, "getMe", {})


async def get_webhook_info(tenant: dict) -> dict:
    return await _call(tenant, "getWebhookInfo", {})


async def set_webhook(tenant: dict, base_url: str) -> dict:
    ...
    return {"webhook_url": url, "result": await _call(tenant, "setWebhook", payload)}


async def delete_webhook(tenant: dict) -> dict:
    return await _call(tenant, "deleteWebhook", {"drop_pending_updates": False})


# ---------- long polling (solo desarrollo local) ----------
async def poll_loop(tenant: dict, stop: asyncio.Event | None = None) -> None:
    import traceback
    await delete_webhook(tenant)  # polling y webhook son mutuamente excluyentes
    offset = None
    ...
    while not (stop and stop.is_set()):
        ...
        data = await _call(tenant, "getUpdates", payload)
        if not data.get("ok"):
        ...
        for upd in updates:
            ...
            try:
                res = await process_update(tenant, upd)
...
async def start_pollers() -> list:
    tasks = []
    for tid in T.all_tenants():
        t = T.get_tenant(tid)
        if t and T.telegram_token(t):
            tasks.append(asyncio.create_task(poll_loop(t)))
    return tasks
