...
CREATE INDEX IF NOT EXISTS idx_msg_conv ON messages(tenant_id, conversation_id, id);
...
CREATE INDEX IF NOT EXISTS idx_appointments_tenant ON appointments(tenant_id, scheduled_for DESC);
...
CREATE INDEX IF NOT EXISTS idx_secret_rotations_tenant ON secret_rotations(tenant_id, rotated_at DESC);
"""
...
def get_or_create_conversation(tenant_id: str, user_ref: str, channel: str,
    ...
    with connect() as con:
        ...
        row = con.execute(
            "SELECT id FROM conversations WHERE tenant_id=? AND user_ref=? AND channel=? "
            "AND status!='closed' ORDER BY updated_at DESC LIMIT ?",
            (tenant_id, user_ref, channel, 1)).fetchone()
        if row and not conversation_id:
            return row["id"]
        cid = conversation_id or uuid.uuid4().hex[:12]
        con.execute(
            "INSERT INTO conversations (id, tenant_id, channel, user_ref, started_at, updated_at) "
            "VALUES (?,?,?,?,?,?)", (cid, tenant_id, channel, user_ref, _now(), _now()))
        return cid
...
def add_message(tenant_id: str, conversation_id: str, role: str, content: str,
    ...
    with connect() as con:
        con.execute(
            "INSERT INTO messages (conversation_id, tenant_id, role, content, media_json, created_at) "
            "VALUES (?,?,?,?,?,?)", (conversation_id, tenant_id, role, content, media_json, _now()))
        con.execute(
            "UPDATE conversations SET updated_at=? WHERE id=? AND tenant_id=?",
            (_now(), conversation_id, tenant_id))


def get_history(tenant_id: str, conversation_id: str, limit: int = 12) -> list[dict]:
    with connect() as con:
        rows = con.execute(
            "SELECT role, content FROM messages WHERE conversation_id=? AND tenant_id=? "
            "ORDER BY id DESC LIMIT ?", (conversation_id, tenant_id, limit)).fetchall()
    return [dict(r) for r in reversed(rows)]


def set_conversation_status(tenant_id: str, conversation_id: str, status: str) -> None:
    with connect() as con:
        con.execute("UPDATE conversations SET status=?, updated_at=? WHERE id=? AND tenant_id=?",
                    (status, _now(), conversation_id, tenant_id))


def get_conversation(tenant_id: str, conversation_id: str) -> dict | None:
    with connect() as con:
        row = con.execute("SELECT * FROM conversations WHERE id=? AND tenant_id=?",
                          (conversation_id, tenant_id)).fetchone()
    if not row:
        return None
    d = _rowdict(row)
...
def is_first_contact(tenant_id: str, user_ref: str) -> bool:
    """True si este user_ref aún no tiene ningún mensaje suyo (lead nuevo).
    Llamar ANTES de guardar el mensaje entrante."""
    with connect() as con:
        row = con.execute(
            "SELECT COUNT(*) AS n FROM messages m "
            "JOIN conversations c ON m.conversation_id=c.id AND m.tenant_id=c.tenant_id "
            "WHERE c.tenant_id=? AND c.user_ref=? AND m.role='user'",
            (tenant_id, user_ref)).fetchone()
    n = row["n"] if not isinstance(row, tuple) else row[0]
    return int(n or 0) == 0


def get_lead_data(tenant_id: str, conversation_id: str) -> dict:
    conv = get_conversation(tenant_id, conversation_id)
    return conv.get("lead_data", {}) if conv else {}


def merge_lead_data(tenant_id: str, conversation_id: str, updates: dict) -> dict:
    ...
    for k, v in updates.items():
        if v is not None and v != "":
            current[k] = v
    with connect() as con:
        con.execute("UPDATE conversations SET lead_data=?, updated_at=? WHERE id=? AND tenant_id=?",
                    (json.dumps(current, ensure_ascii=False), _now(), conversation_id, tenant_id))
    return current


def conversation_status(tenant_id: str, conversation_id: str) -> str | None:
    conv = get_conversation(tenant_id, conversation_id)
    return conv.get("status") if conv else None


def assign_conversation(tenant_id: str, conversation_id: str, email: str | None) -> None:
    """Deriva la conversación a un asesor del panel (o None para desasignar)."""
    with connect() as con:
        con.execute("UPDATE conversations SET assigned_to=?, updated_at=? WHERE id=? AND tenant_id=?",
                    (email, _now(), conversation_id, tenant_id))


def handoff_load_by_agent(tenant_id: str) -> dict[str, int]:
    ...
    with connect() as con:
        rows = con.execute(
            ...
            "WHERE tenant_id=? AND status='handoff' AND assigned_to IS NOT NULL "
            "GROUP BY assigned_to", (tenant_id,)).fetchall()
    return {r["assigned_to"]: int(r["n"]) for r in rows}


def list_conversations(tenant_id: str, limit: int = 50, assigned_to: str | None = None,
                       include_unassigned: bool = False) -> list[dict]:
    """Conversaciones del tenant. Si `assigned_to` se da (vista de asesor), filtra a
    ...
    with connect() as con:
        rows = con.execute(
            ...
            "AND m.tenant_id=c.tenant_id ORDER BY m.id DESC LIMIT 1) AS last_message, "
            "(SELECT COUNT(*) FROM messages m WHERE m.conversation_id=c.id "
            "AND m.tenant_id=c.tenant_id) AS message_count "
...
def add_usage(tenant_id: str, conversation_id: str | None, provider: str, model: str,
              tokens_in: int, tokens_out: int, cost_usd: float) -> None:
    with connect() as con:
        con.execute(
            "INSERT INTO usage_events (tenant_id, conversation_id, provider, model, tokens_in, "
            "tokens_out, cost_usd, created_at) VALUES (?,?,?,?,?,?,?,?)",
            (tenant_id, conversation_id, provider, model, tokens_in, tokens_out, cost_usd, _now()))
...
def usage_summary(tenant_id: str | None = None) -> list[dict]:
    q = ("SELECT tenant_id, COUNT(*) AS calls, SUM(tokens_in) AS tokens_in, "
...
def usage_events(tenant_id: str | None = None, limit: int = 100) -> list[dict]:
...
def add_audit_event(tenant_id: str | None, actor: str | None,
    ...
    with connect() as con:
        con.execute(
            "INSERT INTO audit_events (tenant_id, actor, action, resource, metadata, created_at) "
            "VALUES (?,?,?,?,?,?)",
            (tenant_id, actor, action, resource, metadata, _now()))


def create_appointment(tenant_id: str, conversation_id: str | None, user_ref: str | None,
    ...
    with connect() as con:
        con.execute(
            "INSERT INTO appointments (tenant_id, conversation_id, user_ref, patient_name, "
            "document_id, specialty, scheduled_for, status, metadata, created_at, updated_at) "
            "VALUES (?,?,?,?,?,?,?,?,?,?,?)",
            (tenant_id, conversation_id, user_ref, patient_name, document_id, specialty,
             scheduled_for, "scheduled", metadata, now, now))
        row = con.execute(
            "SELECT * FROM appointments WHERE tenant_id=? ORDER BY id DESC LIMIT ?",
            (tenant_id, 1)).fetchone()
    return _rowdict(row)


def list_appointments(tenant_id: str, limit: int = 100) -> list[dict]:
    with connect() as con:
        rows = con.execute(
            "SELECT * FROM appointments WHERE tenant_id=? ORDER BY scheduled_for DESC LIMIT ?",
            (tenant_id, limit)).fetchall()
    return [dict(r) for r in rows]


def export_conversations(tenant_id: str, limit: int = 500) -> list[dict]:
    """Conversaciones + sus mensajes de un tenant (para offboarding/portabilidad)."""
    out = []
...
def usage_daily(tenant_id: str | None = None) -> list[dict]:
    """Agregación por (tenant, día) calculada en Python — sin SQL dependiente del backend."""
    agg: dict[tuple, dict] = {}
    for r in usage_events(tenant_id, limit=100000):
        # created_at es TEXT en SQLite y datetime en Postgres -> str() antes de cortar.
        day = str(r.get("created_at") or "")[:10]
        if not day:
            continue
        key = (r["tenant_id"], day)
        a = agg.setdefault(key, {"tenant_id": r["tenant_id"], "day": day,
    ...
    rows = sorted(agg.values(), key=lambda x: (x["tenant_id"], x["day"]), reverse=True)
    for a in rows:
...
def count_by_tenant(table: str) -> list[dict]:
    ...
    with connect() as con:
        rows = con.execute(
            f"SELECT tenant_id, COUNT(*) AS count FROM {table} GROUP BY tenant_id ORDER BY tenant_id"
        ).fetchall()
    return [dict(r) for r in rows]


def add_secret_rotation(tenant_id: str, actor: str | None, secret_path: str,
                        env_name: str, note: str | None = None) -> dict:
    with connect() as con:
        con.execute(
            "INSERT INTO secret_rotations (tenant_id, actor, secret_path, env_name, note, rotated_at) "
            "VALUES (?,?,?,?,?,?)",
            (tenant_id, actor, secret_path, env_name, note, _now()))
        row = con.execute(
            "SELECT * FROM secret_rotations WHERE tenant_id=? ORDER BY id DESC LIMIT ?",
            (tenant_id, 1)).fetchone()
    return _rowdict(row)


def list_secret_rotations(tenant_id: str, limit: int = 100) -> list[dict]:
    with connect() as con:
        rows = con.execute(
            "SELECT * FROM secret_rotations WHERE tenant_id=? ORDER BY rotated_at DESC LIMIT ?",
            (tenant_id, limit)).fetchall()
    return [dict(r) for r in rows]
